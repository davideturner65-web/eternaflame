...
async function getDiscoverProfile(): Promise<{
  ...
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_SUPABASE_URL}/functions/v1/discover`,
      { cache: "no-store" }
    );
    if (!res.ok) throw new Error();
    return res.json();
  } catch {
    // Fallback: pull directly from Supabase
    try {
      const supabase = createClient();
...
export default async function Home() {
  const [recentProfiles, discover] = await Promise.all([
    getRecentProfiles(),
    getDiscoverProfile(),
  ]);
    ...
    <div className="flex flex-col">
      ...
      <section className="relative flex flex-col items-center justify-center text-center px-4 py-24 sm:py-36 overflow-hidden">
        <div className="absolute inset-0 pointer-events-none" style={{
          background: "radial-gradient(ellipse 70% 50% at 50% 40%, rgba(196,164,105,0.07) 0%, transparent 70%)",
        }} />
        <p className="text-[#C4A869] text-xs tracking-[0.2em] uppercase mb-5 font-semibold">
          An index of every human life
        </p>
        <h1 className="text-5xl sm:text-6xl md:text-7xl font-bold text-[#E7E0D5] leading-tight max-w-3xl"
        ...
        <p className="mt-6 text-[#A8A29E] text-lg sm:text-xl max-w-2xl leading-relaxed">
          ...
          <span className="text-[#E7E0D5]">Remembering someone shouldn&rsquo;t cost anything.</span>
        </p>
        <div className="flex flex-col sm:flex-row items-center gap-4 mt-10">
          <Link href="/create"
            className="px-7 py-3.5 rounded-button text-[#1C1917] font-semibold text-base transition-all duration-200 hover:opacity-90"
      ...
      <section className="max-w-6xl mx-auto px-4 sm:px-6 py-20 w-full">
        <div className="flex items-baseline justify-between mb-8">
          <div>
            ...
            <h2 className="text-2xl sm:text-3xl text-[#E7E0D5] font-semibold"
              style={{ fontFamily: "var(--font-playfair)" }}>
              Meet someone you&rsquo;ve never known
            </h2>
          </div>
          <Link href="/discover" className="text-sm text-[#A8A29E] hover:text-[#C4A869] transition-colors hidden sm:block">
      ...
      <section className="border-t border-[rgba(196,164,105,0.1)] py-20">
        <div className="max-w-6xl mx-auto px-4 sm:px-6">
          ...
          <h2 className="text-2xl sm:text-3xl text-[#E7E0D5] font-semibold text-center mb-12"
            style={{ fontFamily: "var(--font-playfair)" }}>
            A life preserved in three steps
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              { number: "01", title: "Create a profile", body: "Add your loved one's name, dates, story, and the places that mattered to them. Completely free." },
              { number: "02", title: "Build their story", body: "Family and friends add memories, connect relatives, and pin locations on the map. A life, not just a listing." },
              ...
              <div key={step.number} className="card-surface rounded-card p-7">
                ...
                <h3 className="text-[#E7E0D5] text-lg font-semibold mb-3"
        ...
        <section className="border-t border-[rgba(196,164,105,0.1)] py-20">
          <div className="max-w-6xl mx-auto px-4 sm:px-6">
            <div className="flex items-baseline justify-between mb-8">
              ...
              <Link href="/search" className="text-sm text-[#A8A29E] hover:text-[#C4A869] transition-colors">
                View all &rarr;
              </Link>
            </div>
      ...
      <section className="border-t border-[rgba(196,164,105,0.1)] py-20">
        <div className="max-w-3xl mx-auto px-4 sm:px-6 text-center">
          <p className="text-xl sm:text-2xl text-[#A8A29E] italic leading-relaxed"
            style={{ fontFamily: "var(--font-playfair)" }}>
