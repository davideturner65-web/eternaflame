...
export const maxDuration = 300; // 5 min — Vercel Pro max; hobby is 60s
...
export async function GET(request: NextRequest) {
  ...
  const windowIndex = parseInt(searchParams.get("window_index") ?? String(dayOfYear % DEATH_YEAR_WINDOWS.length));
  ...
  try {
    ...
    if (!response.ok) {
      ...
      console.error("SPARQL error:", response.status, text.slice(0, 300));
    ...
    console.log(`Wikidata: got ${rows.length} rows, window ${win.from}–${win.to}`);
